...
-- Rule: No Data Logged
insert into rules (rule_desc, rule_query, rule_message, rule_shellcmd, rule_active) values('No Data Logged', 'select date_format( x.ts, \'%M %d, %Y at %H:%i:%s\' ) from ( select max(timestamp) as ts from sensor_data ) x where timestampdiff( MINUTE, x.ts, now() ) > 5;', 'No data has been logged for at least 5 minutes. Last entry was on %s. The Raspberry Pi will be rebooted...', 'sudo reboot', Y');
